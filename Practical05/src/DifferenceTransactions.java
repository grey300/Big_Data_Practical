...
public class DifferenceTransactions {
    ...
    public static class DifferenceReducer extends Reducer<Text, Text, NullWritable, Text> {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 3) {
            System.err.println("Usage: DifferenceTransactions <input1> <input2> <output>");
            System.exit(2);
        }

        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf, "difference transactions");
        job.setJarByClass(DifferenceTransactions.class);
